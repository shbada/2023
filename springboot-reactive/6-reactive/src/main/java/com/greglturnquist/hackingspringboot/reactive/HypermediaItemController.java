...
public class HypermediaItemController {
	...
	Mono<EntityModel<Item>> findOne(@PathVariable String id) {
		...
		return Mono.zip(repository.findById(id), selfLink, aggregateLink) // <4> 여러개의 비동기 요청 결과를 하나로 합친다.
				.map(o -> EntityModel.of(o.getT1(), Links.of(o.getT2(), o.getT3()))); // <5> Mono로 감싸서 반환
	}
	// end::find-one[]
